...
This repo contains the SLiM model of TE dynamics developed at WIAS, 2024 in Berlin. 
...
Simulations on the cluster will probably have to be run with slurm or similar using the single command shown above. 
